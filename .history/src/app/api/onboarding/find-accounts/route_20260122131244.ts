...
export async function POST(req: Request) {
  ...
  try {
    ...
    // ─────────────────────────────────────────────────────────────
    // FIXED: Proper protobuf message construction
    // ─────────────────────────────────────────────────────────────

    // 1. Create inner AccountListRequest
    ...
    // 2. Pack into Any (this is the key fix — binary value, not object)
    const anyPayload = AnyType.create({
    type_url: 'type.googleapis.com/accountlist.AccountListRequest',
    value: innerBuffer
  }) as protobuf.Message<{ type_url: string; value: Uint8Array }>;
console.log('anyPayload.value length:', (anyPayload as any).value?.length ?? 0);
  // 3. Create outer ServiceRequest
    ...
    // Debug logging — compare this base64 to server received preview
    ...
    // 4. Call gRPC with real protobuf object
