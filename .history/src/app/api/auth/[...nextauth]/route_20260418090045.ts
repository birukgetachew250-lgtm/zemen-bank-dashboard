...
let cachedTimeoutMinutes = DEFAULT_SESSION_TIMEOUT_MINUTES;
let cachedAt = 0;

async function getSessionTimeoutMinutes(): Promise<number> {
  const now = Date.now();
  if (now - cachedAt < POLICY_CACHE_TTL_MS) {
    return cachedTimeoutMinutes;
  }

  try {
    const policy = await db.securityPolicy.findUnique({ where: { id: 1 } });
    cachedTimeoutMinutes = Math.max(5, Number(policy?.sessionTimeout || DEFAULT_SESSION_TIMEOUT_MINUTES));
  } catch (e) {
    cachedTimeoutMinutes = DEFAULT_SESSION_TIMEOUT_MINUTES;
  }
...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      ...
      const inactivityLimitMs = timeoutMinutes * 60 * 1000;
      token.sessionExpired = now - lastActivityAt > inactivityLimitMs;
