...
export async function GET() {
  try {
    const [
      ...
      // ── Slowest endpoints (avg response time, min 10 calls) ─────────────
      executeQuery(CS, `
      ...
      // ── Avg response time per service (last 7 days) ──────────────────────
      executeQuery(CS, `
        SELECT SERVICE_NAME,
          ROUND(AVG(RESPONSE_TIME_MS)) AS AVG_MS,
          COUNT(*) AS CALL_COUNT,
      ...
      executeQuery(CS, `
        SELECT * FROM (
          SELECT SERVICE_NAME, ENDPOINT_URL, STATUS, ERROR_MESSAGE, CREATED_DATE
          FROM ${LOGS}
          WHERE STATUS IN ('FAILED','ERROR')
    ...
    const totalAll: any = await executeQuery(CS, `
