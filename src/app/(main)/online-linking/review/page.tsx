...
import {
  Clock, Eye, Search, RefreshCw, ChevronLeft, ChevronRight,
  ScanFace, CheckCircle2, XCircle,
} from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-500/20 to-orange-500/20 border border-white/10 flex items-center justify-center">
          ...
          <div>
            ...
            <p className="text-sm text-white/40">
              Pending requests awaiting your review · Step 1 of 2
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span className="px-3 py-1.5 rounded-full text-xs font-medium bg-amber-500/15 text-amber-400 border border-amber-500/20">
            {total} pending
          </span>
          <button
            onClick={() => fetchRows()}
      ...
      {/* Info Banner */}
      <div className="bg-gradient-to-r from-amber-500/5 to-orange-500/5 border border-amber-500/10 rounded-xl px-5 py-3 flex items-center gap-3">
        <ScanFace size={18} className="text-amber-400 shrink-0" />
        <p className="text-sm text-white/50">
          Review all customer information, verification data, images, and liveness video before submitting your review.
          <span className="text-amber-400"> Downloads are disabled for privacy.</span>
        </p>
      </div>
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2.5 text-sm bg-white/[0.04] border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:border-amber-500/40 focus:ring-1 focus:ring-amber-500/20"
        />
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5">
                {['Name', 'CIF', 'Phone', 'Branch', 'Account', 'Fayda', 'Liveness', 'Score', 'Submitted', 'Action'].map((h) => (
                ...
                <tr>
                  <td colSpan={10} className="text-center py-16">
                    <div className="flex flex-col items-center gap-3 text-white/30">
                      ...
                      <p className="text-xs text-white/20">All caught up! Check back later.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                rows.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5">
                        <span className={`inline-flex items-center gap-1 text-xs ${row.LivenessCheckPassed ? 'text-emerald-400' : 'text-red-400'}`}>
                          {row.LivenessCheckPassed ? <CheckCircle2 size={12} /> : <XCircle size={12} />}
                        </span>
                      </td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-4 py-3.5">
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-gradient-to-r from-sky-500/20 to-blue-500/20 text-sky-400 border border-sky-500/20 hover:border-sky-500/40 hover:bg-sky-500/10 transition-all"
                        >
                          <Eye size={12} />
                          Review
                        </button>
