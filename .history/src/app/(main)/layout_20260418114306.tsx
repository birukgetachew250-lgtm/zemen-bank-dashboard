...
export default function MainLayout({
  ...
  useEffect(() => {
    ...
    // Only refresh token halfway through timeout period (e.g., 15 min timeout = refresh at 7.5 min) to avoid excessive re-renders and page flicker
    const TOKEN_REFRESH_INTERVAL = (timeoutMinutes * 60 * 1000) / 2;
    ...
    const touchActivity = () => {
      ...
      // Replace fixed 60s throttle with timeout-aware refresh interval to reduce flicker
      if (now - lastTouchForToken > TOKEN_REFRESH_INTERVAL) {
    ...
    // Check every 30s instead of 15s to reduce polling overhead and page reflow
