        ...
        switch (approval.type) {
             case 'new-customer':
                ...
                for (const acc of linkedAccounts) {
                    ...
                    const accBinds = {
                        ...
                        HashedAccountNumber: hashSha256(acc.CUSTACNO, 'Linked account number'),
                        FirstName: encrypt(firstName)!,
                        SecondName: encrypt(secondName)!,
                        LastName: encrypt(lastName)!,
            ...
            case 'customer-account':
                ...
                for (const acc of accountsToLink) {
                    const rawAccountNumber = acc.CUSTACNO || acc.accountNumber;
                    const rawAccountType = acc.ACCLASSDESC || acc.accountType || 'Unknown';
                    const rawCurrency = acc.CCY || acc.currency || 'ETB';
                    const rawBranchCode = acc.BRANCH_CODE || acc.branchCode || '';
