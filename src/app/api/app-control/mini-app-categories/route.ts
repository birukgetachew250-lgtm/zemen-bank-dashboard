...
export async function POST(req: Request) {
  ...
  try {
    ...
    await executeQuery(CS, `INSERT INTO ${TABLE} ("Id","Name","IconName","ColorHex","Description","Status","Rank","CreatedAt","UpdatedAt") VALUES (:id,:name,:icon,:color,:descr,:status,:rank,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`, {
      id, name: b.Name, icon: b.IconName || '', color: b.ColorHex || '#808080', descr: b.Description || null, status: b.Status || 'Active', rank: b.Rank || 0
    });
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    const map: Record<string, string> = { Name:'name',IconName:'icon',ColorHex:'color',Description:'b_desc',Status:'status',Rank:'rank' };
