...
export function PeriodIntervalsClient({
  ...
  const openEditDialog = (item: Interval) => {
    setEditingItem(item);
    setNewItem({ code: item.code, name: item.name, days: String(item.days) });
    setIsDialogOpen(true);
  };
  ...
  return (
    <>
      ...
      {isDialogOpen && (
        <Card className="mt-6">
          <CardHeader>
            <CardTitle>{editingItem ? 'Edit Period Interval' : 'Add New Period Interval'}</CardTitle>
            <CardDescription>Define interval metadata and save your changes.</CardDescription>
          </CardHeader>
