...
interface UserActionClientProps {
    action: 'ResetPassword' | 'UnlockUser';
}
...
export function UserActionClient({ action }: UserActionClientProps) {
  ...
  const pageConfig = {
      ...
      'UnlockUser': { title: 'Unlock User', description: 'Unlock a locked user account and reset failed attempts.', buttonLabel: 'Unlock User', icon: <KeyRound className="mr-2 h-4 w-4" />, actionName: 'unlock' },
  }[action];
