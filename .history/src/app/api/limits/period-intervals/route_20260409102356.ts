...
export async function POST(req: Request) {
    try {
        ...
        const query = `INSERT INTO ${TABLE} ("Id", "Code", "Name", "Days", "Version") VALUES (:Id, :Code, :Name, :Days, SUBSTR(RAWTOHEX(SYS_GUID()), 1, 8))`;
...
export async function PUT(req: Request) {
    try {
        ...
        if (!id || !code || !name || days === undefined) {
            return NextResponse.json({ message: 'ID, Code, Name, and Days are required' }, { status: 400 });
        }
        const query = `UPDATE ${TABLE} SET "Code" = :Code, "Name" = :Name, "Days" = :Days, "UpdateDate" = SYSTIMESTAMP WHERE "Id" = :Id`;
        await executeQuery(process.env.LIMIT_CHARGE_MODULE_DB_CONNECTION_STRING, query, { Id: id, Code: code, Name: name, Days: parseInt(days, 10) });
        return NextResponse.json({ id, code, name, days: parseInt(days, 10) });
    } catch (error) {
        console.error('Failed to update period interval:', error);
        return NextResponse.json({ message: 'Internal Server Error' }, { status: 500 });
    }
}
