...
export async function POST(req: Request) {
  try {
    ...
    const disasterRiskColumn = await resolveDisasterRiskColumn();
    const disasterRiskInsertColumn = disasterRiskColumn ? `, ${quoteIdentifier(disasterRiskColumn)}` : '';
    const disasterRiskInsertValue = disasterRiskColumn ? ', :DisasterRiskPercentage' : '';

    const resolvedCategoryIds = normalizeCategoryIds(categoryIds, categoryId);
    const id = crypto.randomUUID();
    const query = `
      INSERT INTO ${TABLE} (
        "Id", "CustomerCategoryId", "TransactionTypeId", "ServiceName", "ChargeType", "Percentage", "FixedAmount", "VatPercentage"${disasterRiskInsertColumn}, "MinCharge", "MaxCharge", "EffectiveFrom", "EffectiveTo", "IsActive", "InsertDate", "Version"
      )
      VALUES (
        :Id, :CustomerCategoryId, :TransactionTypeId, :ServiceName, :ChargeType, :Percentage, :FixedAmount, :VatPercentage${disasterRiskInsertValue}, :MinCharge, :MaxCharge,
    ...
    const binds = {
      ...
      ...(disasterRiskColumn
        ? {
            DisasterRiskPercentage:
              disasterRiskPercentage !== undefined && disasterRiskPercentage !== ''
                ? parseFloat(disasterRiskPercentage)
                : null,
          }
        : {}),
    ...
    return NextResponse.json(
      {
        ...
        disasterRiskPercentage: disasterRiskColumn ? (binds as any).DisasterRiskPercentage ?? null : null,
...
export async function PUT(req: Request) {
  try {
    ...
    const disasterRiskColumn = await resolveDisasterRiskColumn();
    const disasterRiskUpdate = disasterRiskColumn
      ? `\n        ${quoteIdentifier(disasterRiskColumn)} = :DisasterRiskPercentage,`
      : '';
