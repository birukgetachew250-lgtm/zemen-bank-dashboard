...
export async function POST(req: Request) {
    try {
        const { categoryId, transactionTypeId, serviceName, isGlobal, currency, effectiveFrom, effectiveTo, limits, perTransactionLimit } = await req.json();
        
        const limitRuleId = crypto.randomUUID();
        const ruleQuery = `
            INSERT INTO ${TABLE} ("Id", "CustomerCategoryId", "TransactionTypeId", "ServiceName", "IsGlobal", "Currency", "EffectiveFrom", "EffectiveTo", "PerTransactionLimit", "IsActive", "InsertDate", "Version") 
            VALUES (:Id, :CustomerCategoryId, :TransactionTypeId, :ServiceName, :IsGlobal, :Currency, CASE WHEN :EffectiveFrom IS NOT NULL THEN TO_TIMESTAMP(:EffectiveFrom, 'YYYY-MM-DD"T"HH24:MI') ELSE NULL END, CASE WHEN :EffectiveTo IS NOT NULL THEN TO_TIMESTAMP(:EffectiveTo, 'YYYY-MM-DD"T"HH24:MI') ELSE NULL END, :PerTransactionLimit, 1, SYSTIMESTAMP, SUBSTR(RAWTOHEX(SYS_GUID()), 1, 8))
        `;
...
export async function PUT(req: Request) {
    try {
        const { id, categoryId, transactionTypeId, serviceName, isGlobal, currency, effectiveFrom, effectiveTo, limits, perTransactionLimit } = await req.json();
