...
export async function POST(req: Request) {
  try {
    ...
    await executeQuery(CS, `INSERT INTO ${TABLE} ("FeeId","CategoryId","FeeName","FeeCode","TransactionType","TransactionSubType","Description","FeeAmount","IsFree","FeePercentage","MinFeeAmount","MaxFeeAmount","MinTransactionAmount","MaxTransactionAmount","Currency","FeeFrequency","EffectiveFrom","EffectiveTo","IsVATApplicable","VATPercentage","IsWaivedForPremium","FreeTransactionsLimit","FreeTransactionsPeriod","Notes","Status","Rank","CreatedBy","UpdatedBy") VALUES (:id,:catId,:name,:code,:txnType,:txnSubType,:desc,:amt,:isFree,:pct,:minFee,:maxFee,:minTxn,:maxTxn,:currency,:freq,:effFrom,:effTo,:vat,:vatPct,:waived,:freeTxnLimit,:freeTxnPeriod,:notes,:status,:rank,:createdBy,:updatedBy)`, {
      id, catId: b.CategoryId, name: b.FeeName, code: b.FeeCode, txnType: b.TransactionType,
      txnSubType: b.TransactionSubType || null, desc: b.Description || null, amt: b.FeeAmount || 0,
