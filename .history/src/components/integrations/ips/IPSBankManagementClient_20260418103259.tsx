...
export function IPSBankManagementClient({ initialBanks }: IPSBankManagementClientProps) {
  ...
  const openAddDialog = () => {
    ...
    setBankData({ bankName: '', bankCode: '', reconciliationAccount: '', bankLogo: '', branchCode: '', status: 'Active', rank: String(banks.length + 1) });
  ...
  const handleSave = async () => {
    if (!bankData.bankName || !bankData.bankCode || !bankData.reconciliationAccount || !bankData.branchCode || !bankData.rank) {
      toast({ variant: 'destructive', title: 'Missing fields', description: 'Bank Name, Code, Reconciliation Account, Branch Code and Rank are required.'});
        return;
    }
    ...
    <Card className="w-full">
      <CardHeader className="flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-4">
            <Input 
              placeholder="Search by name, code or branch code..."
                className="max-w-sm"
                value={searchTerm}
            ...
            <TableBody>
              {filteredBanks.length > 0 ? filteredBanks.map((b) => (
                <TableRow key={b.id}>
                  ...
                  <TableCell className="font-mono">{b.branchCode}</TableCell>
        ...
        <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4 py-4">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="branchCode" value={bankData.branchCode} onChange={(e) => handleFieldChange('branchCode', e.target.value)} className="col-span-3" />
                </div>
                 <div className="grid grid-cols-4 items-center gap-4">
