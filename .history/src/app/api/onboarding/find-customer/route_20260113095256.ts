...
export async function POST(req: Request) {
    ...
    try {
        ...
        const anyPayload = {
            ...
            customer_cif: customer_id   // your "0000238" etc.
        };
        const requestPayload = {
        request_id:     `AL-${new Date().toISOString().slice(0,10).replace(/-/g,'')}-${Math.floor(Math.random()*1000).toString().padStart(3,'0')}`,
        source_system:  'MOBILE',
        channel:        'MOBILE',
        user_id:        'some-user-or-empty',   // ask backend what to put
        data:           anyPayload
        };
        console.error("payload request", requestPayload);
        // const requestPayload = {
        //     branch_code,
        //     customer_id
        // };
