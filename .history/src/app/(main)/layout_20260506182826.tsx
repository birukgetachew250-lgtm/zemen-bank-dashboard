...
const FORCE_CHANGE_PASSWORD_PATH = '/settings/change-password';
...
export default function MainLayout({
  ...
  useEffect(() => {
    ...
    if ((session as any)?.mustChangePassword && pathname !== FORCE_CHANGE_PASSWORD_PATH) {
  ...
  useEffect(() => {
    ...
    // Prevent a token refresh on the first interaction right after login.
    if (lastTouchForTokenRef.current === 0) {
      lastTouchForTokenRef.current = Date.now();
    }
  ...
  if ((status === 'loading' && !session) || !session || (session as any)?.mfaRequired) {
    return (
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex-1 flex min-h-screen flex-col">
        ...
        <main className="relative flex-1 overflow-y-auto">
          <Watermark />
          <div className="relative z-10 flex min-h-full flex-col p-4 md:p-6 lg:p-8">
