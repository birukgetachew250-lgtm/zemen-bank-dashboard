...
try {
    ...
    if (grpcObj.accountdetail) {
        console.log(
            '[PROTO] Types inside accountdetail:',
            Object.keys(grpcObj.accountdetail)
        );
    } else {
        console.log('[PROTO] No "accountdetail" package found!');
    }

    // Try to find the message (case insensitive check as fallback)
    const accountdetailPkg = grpcObj.accountdetail || {};
    AccountDetailResponseProto = 
        accountdetailPkg.AccountDetailResponse ||
        accountdetailPkg.accountdetailresponse ||
        accountdetailPkg.Accountdetailresponse;

    if (AccountDetailResponseProto) {
        console.log('[PROTO] SUCCESS - Found AccountDetailResponse');
    } else {
        console.error('[PROTO] FAILED - AccountDetailResponse not found');
        console.error('Possible message names you might see:', 
            Object.keys(accountdetailPkg).join(', ') || 'none');
    }
...
export async function POST(req: Request) {
    ...
    try {
        ...
        const dataAny = grpcResponse.data;

        if (!dataAny || !dataAny.value) {
            throw new Error("Success but no data.value received");
        }

        if (!AccountDetailResponseProto) {
            throw new Error("AccountDetailResponseProto not available (even though debug log said yes)");
        }
        
        let buffer: Buffer;
        if (Buffer.isBuffer(dataAny.value)) {
            buffer = dataAny.value;
        } else if (dataAny.value instanceof Uint8Array) {
            buffer = Buffer.from(dataAny.value);
        } else if (typeof dataAny.value === 'string') {
            buffer = Buffer.from(dataAny.value, 'base64'); // if server sent base64
        } else {
            throw new Error(`Unsupported data.value type: ${typeof dataAny.value}`);
        }
        console.log('[DEBUG] Buffer length for decoding:', buffer.length);
        const decodedResponse = AccountDetailResponseProto.decode(buffer);

        const responseObj = AccountDetailResponseProto.toObject(decodedResponse, {
            longs: String,
            enums: String,
            bytes: String,
            defaults: true,
        ...
        console.info('Decoded AccountDetailResponseProto', responseObj);
        const customer = responseObj.customer || null;

        // Optional: log the full decoded structure for verification
        console.log('[DECODE SUCCESS] Decoded AccountDetailResponse:', responseObj);

        return NextResponse.json({
            success: grpcResponse.success,
            message: grpcResponse.message || "Customer details fetched successfully",
            code: grpcResponse.code,
            customer: customer,
            status: responseObj.status,
            serverMessage: responseObj.message,
