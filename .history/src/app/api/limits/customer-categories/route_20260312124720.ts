...
export async function PUT(req: Request) {
    try {
        ...
        if (!id || !code || !name) {
            return NextResponse.json({ message: 'ID, Code and Name are required' }, { status: 400 });
        }
        const query = `UPDATE ${TABLE} SET "Code" = :Code, "Name" = :Name, "Description" = :Description, "UpdateDate" = SYSTIMESTAMP WHERE "Id" = :Id`;
        await executeQuery(process.env.LIMIT_CHARGE_MODULE_DB_CONNECTION_STRING, query, { Id: id, Code: code, Name: name, Description: description || null });
        return NextResponse.json({ id, code, name, description });
    } catch (error) {
