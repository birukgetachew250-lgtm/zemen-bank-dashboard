...
export async function POST(req: Request) {
    ...
    try {
        ...
        // Pack as Any – the @type must match exactly what server expects
        const dataAny = {
            type_url: 'type.googleapis.com/accountdetail.AccountDetailRequest', // ← very important
            value: Buffer.from(JSON.stringify(innerPayload)) // simple way (not ideal, but works for many servers)
            // Better: use real protobuf serialization if you generate messages (see below)
        };

        const request = {
            request_id: `AL-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${Math.floor(1000 + Math.random() * 9000)}`,
            source_system: 'MOBILE',
            channel: 'MOBILE',
            user_id: 'onboarding_system', // ← ASK TEAM: what value should be here?
            data: dataAny
        };
        ...
        if (grpcResponse?.data?.value) {
            // Quick & dirty JSON parse (many FLEX implementations use JSON inside Any)
            try {
                customerData = JSON.parse(Buffer.from(grpcResponse.data.value).toString());
            } catch (e) {
                console.warn('Could not parse Any.value as JSON', e);
            }
        }
        ...
        return NextResponse.json({
            ...
            customer: customerData?.customer || customerData, // adjust based on real response
